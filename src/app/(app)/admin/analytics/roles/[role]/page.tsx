...
import { Users, Briefcase, FileText, Star, CheckCircle } from "lucide-react";
import { 
...
import { Code, Palette, Megaphone, Database } from 'lucide-react';
...
export default async function RoleAnalyticsPage({ params }: { params: { role: string } }) {
    const roleName = capitalize(params.role) as RoleCategory;
    ...
    const [allUsers, allCompanies, allTasks, allSubmissions, allEvaluations] = await Promise.all([
        getUsers(),
        getCompanies(),
        getTasks(),
        getSubmissions(),
    ...
    const roleSubmissions = allSubmissions.filter(s => roleTaskIds.has(s.taskId));
    const roleSubmissionIds = new Set(roleSubmissions.map(s => s.id));
    const roleEvaluations = allEvaluations.filter(e => roleSubmissionIds.has(e.submissionId));
    ...
    const averageScore = evaluatedCount > 0 
        ? Math.round(roleEvaluations.reduce((acc, curr) => acc + curr.score, 0) / evaluatedCount)
        : 0;
