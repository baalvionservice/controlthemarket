...
const subscriptionStatuses: Subscription['status'][] = ['ACTIVE', 'CANCELED', 'EXPIRED', 'TRIAL'];
const billingCycles: Subscription['billingCycle'][] = ['MONTHLY', 'YEARLY'];

const formSchema = z.object({
  planId: z.string({ required_error: 'Please select a plan.' }),
  status: z.enum(subscriptionStatuses, { required_error: 'Please select a status.'}),
  billingCycle: z.enum(billingCycles, { required_error: 'Please select a billing cycle.'}),
});
...
export function SubscriptionManagementDialog({ isOpen, onOpenChange, onSave, company, allPlans, currentSubscription }: SubscriptionManagementDialogProps) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const updatedSubscription: Subscription = {
        ...
        ...values,
        endDate: values.billingCycle === 'YEARLY' 
                        ...
                        <FormItem>
                            <FormLabel>Subscription Plan</FormLabel>
                             <Select onValueChange={field.onChange} value={field.value}>
                                ...
                                <SelectContent>
                                    {allPlans.map(plan => (
                                        <SelectItem key={plan.id} value={plan.id}>{plan.name} (${plan.priceMonthly}/mo)</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <FormMessage />
