    ...
    useEffect(() => {
        ...
        if (ids && ids.length > 0) {
            const fetchData = async () => {
                setLoading(true);
                try {
                    const submissions = await Promise.all(ids.map(id => getSubmission(id).then(sub => sub as Submission)));
                    ...
                    setData(detailedData.filter((d): d is ComparisonData => !!d.task && !!d.candidate));
                } catch (error) {
...
export default function ComparePage() {
  return (
    <Suspense fallback={<div className="flex h-full w-full items-center justify-center p-8"><Loader2 className="h-8 w-8 animate-spin" /></div>}>
      <ComparePageContent />
    </Suspense>
  );
}
