...
export function EducationMonetizationDashboard({ data }: MonetizationDashboardProps) {
  ...
  const studentLimit = typeof currentPlanDetails?.students === 'number' ? currentPlanDetails.students : 1000;
  const studentUsagePercentage = Math.round((current_subscription.active_students / studentLimit) * 100);
    ...
    <div className="space-y-8 mt-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose the plan that best fits your institution's scale and needs.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {subscription_plans.map((plan) => (
                <TableRow key={plan.plan}>
                  ...
                  <TableCell>{plan.students}</TableCell>
                  <TableCell>{plan.features.slice(0, 3).join(', ')}</TableCell>
                  <TableCell>
                    {typeof plan.price === 'number' ? `$${plan.price * 6}/semester` : plan.price}
                  </TableCell>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                <strong>{current_subscription.institution}</strong> is on the <strong>{current_subscription.current_plan}</strong> plan.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
               <div className="space-y-2">
                   <div className="flex justify-between text-sm">
                       <span className="font-medium text-muted-foreground flex items-center gap-2"><Users className="h-4 w-4" /> Active Students</span>
                       <span><strong>{current_subscription.active_students}</strong> / {studentLimit}</span>
                   </div>
                   <Progress value={studentUsagePercentage} />
               </div>
               <div className="space-y-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4 text-xs font-mono bg-muted p-4 rounded-md">
                <p><span className="font-bold text-green-600">GET</span> /api/educational_institutions/subscription_plans</p>
                <p><span className="font-bold text-green-600">GET</span> /api/educational_institutions/&#123;id&#125;/current_subscription</p>
                <p><span className="font-bold text-blue-600">POST</span> /api/educational_institutions/&#123;id&#125;/change_subscription</p>
            </CardContent>
          </Card>
        </div>
